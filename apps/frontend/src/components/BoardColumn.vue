<template>
  <v-card
    class="column-card flex-shrink-0 mr-4 rounded-lg d-flex flex-column pb-2"
    width="280"
  ...
  >
    <v-card-title
      class="text-subtitle-1 font-weight-bold pt-3 pb-2 px-3 d-flex justify-space-between align-center text-grey-darken-3"
    >
      <v-text-field
        v-if="isEditingName"
        v-model="editName"
      ...
      <v-menu>
        ...
        <v-list density="compact">
          ...
          <v-list-item
            ...
            title="Delete List"
            @click="$emit('columnDeleted', column.id)"
          ></v-list-item>
        </v-list>
      </v-menu>
    </v-card-title>
    ...
    <v-card-text
      class="px-3 py-0 overflow-y-auto d-flex flex-column gap-2"
